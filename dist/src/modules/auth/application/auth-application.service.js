...
let AuthApplicationService = class AuthApplicationService {
  ...
  async loginWithPassword(email, password) {
    ...
    if (!user) throw auth_exception_1.AuthException.invalidCredentials();
  ...
  async refreshToken(refreshToken) {
    ...
    if (!record) throw auth_exception_1.AuthException.invalidRefreshToken();
    const user = await this.userRepo.findActiveById(record.userId);
    if (!user) throw auth_exception_1.AuthException.invalidRefreshToken();
  ...
  async register(dto) {
    ...
    if (existing) throw auth_exception_1.AuthException.emailAlreadyExists();
  ...
  async verifyEmail(token) {
    const result = await this.db.db
      ...
      .limit(1);
    const user = result[0];
    if (!user) throw auth_exception_1.AuthException.invalidResetToken();
  ...
  async resetPassword(token, newPassword) {
    ...
    const results = await this.db.db
      ...
      .where(
        (0, drizzle_orm_1.eq)(password_reset_schema_1.passwordResetTokens.tokenHash, tokenHash),
      )
      .limit(1);
    const resetRecord = results[0];
    if (!resetRecord) throw auth_exception_1.AuthException.invalidResetToken();
    ...
    await this.db.db
      .delete(password_reset_schema_1.passwordResetTokens)
      .where((0, drizzle_orm_1.eq)(password_reset_schema_1.passwordResetTokens.id, resetRecord.id));
  }
  ...
  async loginWithMagicLink(token) {
    ...
    if (!magicLinkRecord) throw auth_exception_1.AuthException.invalidMagicLink();
    ...
    if (!user) throw auth_exception_1.AuthException.invalidMagicLink();
    await this.db.db
      .delete(magic_links_schema_1.magicLinks)
      .where((0, drizzle_orm_1.eq)(magic_links_schema_1.magicLinks.id, magicLinkRecord.id));
    return this.generateAuthResult(user.id, user.email, user.name, user.role);
  }
  generateSecureToken() {
  ...
  async incrementFailedLoginAttempts(userId) {
    await this.db.db
      ...
      .set({
        failedLoginAttempts: (0,
        drizzle_orm_1.sql)`${users_schema_1.users.failedLoginAttempts} + 1`,
      })
