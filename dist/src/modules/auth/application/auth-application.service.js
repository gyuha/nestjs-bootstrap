...
let AuthApplicationService = class AuthApplicationService {
    constructor(userRepo, jwtTokenService, tokenRepo, oauthGoogle, oauthKakao, db, env) {
    ...
    async loginWithPassword(email, password) {
        ...
        if (!isValid)
            throw auth_exception_1.AuthException.invalidCredentials();
        if (user.status !== role_value_object_1.UserStatus.ACTIVE)
            throw auth_exception_1.AuthException.accountInactive();
    ...
    async refreshToken(refreshToken) {
        ...
        await this.tokenRepo.storeRefreshToken(this.jwtTokenService.hashToken(newTokenPair.refreshToken), user.id, record.deviceInfo, expiresAt);
        return newTokenPair;
    }
    async generateAuthResult(userId, email, name, role) {
        ...
        const expiresIn = this.env.get('REFRESH_TOKEN_EXPIRES_IN');
        const expiresAt = this.calculateExpiresAt(expiresIn);
        await this.tokenRepo.storeRefreshToken(this.jwtTokenService.hashToken(tokenPair.refreshToken), userId, null, expiresAt);
        return {
...
exports.AuthApplicationService = AuthApplicationService = __decorate([
    ...
    __metadata("design:paramtypes", [Object, Function, Object, Function, Function, Function, Function])
