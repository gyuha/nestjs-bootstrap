...
let PgVectorStoreService = class PgVectorStoreService {
    ...
    async similaritySearch(queryEmbedding, topK) {
        const results = await this.db.db
            ...
            distance: (0, drizzle_orm_1.sql) `${rag_chunks_schema_1.ragChunks.embedding} <=> ${queryEmbedding}`,
        })
            .from(rag_chunks_schema_1.ragChunks)
            .orderBy((0, drizzle_orm_1.sql) `${rag_chunks_schema_1.ragChunks.embedding} <=> ${queryEmbedding}`)
            .limit(topK);
    ...
    async upsert(documents) {
        ...
        await this.db.db.insert(rag_chunks_schema_1.ragChunks).values(chunks).onConflictDoUpdate({
