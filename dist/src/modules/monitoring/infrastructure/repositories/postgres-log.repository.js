...
let PostgresLogRepository = class PostgresLogRepository {
    ...
    async findMany(filters) {
        ...
        if (filters.sessionId) {
            conditions.push((0, drizzle_orm_1.eq)(ai_api_logs_schema_1.aiApiLogs.sessionId, filters.sessionId));
        }
        if (filters.provider) {
            conditions.push((0, drizzle_orm_1.eq)(ai_api_logs_schema_1.aiApiLogs.provider, filters.provider));
        }
        if (filters.startDate) {
            conditions.push((0, drizzle_orm_1.gte)(ai_api_logs_schema_1.aiApiLogs.createdAt, filters.startDate));
        }
        if (filters.endDate) {
            conditions.push((0, drizzle_orm_1.lte)(ai_api_logs_schema_1.aiApiLogs.createdAt, filters.endDate));
        }
    ...
    async aggregateMetrics(filters) {
        ...
        const [result] = await this.drizzle.db
            ...
            totalPromptTokens: (0, drizzle_orm_1.sql) `sum(${ai_token_usage_schema_1.aiTokenUsage.promptTokens})`,
            totalCompletionTokens: (0, drizzle_orm_1.sql) `sum(${ai_token_usage_schema_1.aiTokenUsage.completionTokens})`,
            avgLatencyMs: (0, drizzle_orm_1.sql) `avg(${ai_api_logs_schema_1.aiApiLogs.latencyMs})`,
            errorRate: (0, drizzle_orm_1.sql) `coalesce(avg(case when ${ai_api_logs_schema_1.aiApiLogs.errorCode} is not null then 1 else 0 end), 0)`,
            ragHitRate: (0, drizzle_orm_1.sql) `coalesce(avg(case when ${ai_api_logs_schema_1.aiApiLogs.useRag} = true then ${ai_api_logs_schema_1.aiApiLogs.ragHitRate} else null end), 0)`,
            costUsd: (0, drizzle_orm_1.sql) `coalesce(sum(${ai_token_usage_schema_1.aiTokenUsage.estimatedCostCents}) / 100.0, 0)`,
        })
            .from(ai_api_logs_schema_1.aiApiLogs)
            .leftJoin(ai_token_usage_schema_1.aiTokenUsage, (0, drizzle_orm_1.eq)(ai_api_logs_schema_1.aiApiLogs.traceId, ai_token_usage_schema_1.aiTokenUsage.traceId))
            .where(whereClause);
        return {
